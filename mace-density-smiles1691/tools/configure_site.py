...
def main(argv=None):
    ...
    parser.add_argument("--prep-python", required=True, type=python_path)
    parser.add_argument("--mace-python", required=True, type=python_path)
    parser.add_argument("--classical-lammps", required=True, type=file_path)
    parser.add_argument("--model", required=True, type=file_path)
    parser.add_argument("--mace-launcher", required=True, type=file_path)
    ...
    for name in ("prep_python", "mace_python", "classical_lammps", "mace_launcher"):
    ...
    config = {
        "prepare_argv": command(args.prep_python, "radonpy_prepare.py"),
        "density_argv": command(args.mace_python, "mace_density.py"),
        "preparation": {"lammps_exec": str(args.classical_lammps),
        ...
        "density": {"model_path": str(args.model), "model_sha256": sha(args.model),
    ...
    with args.output.open("x", encoding="utf-8") as out:
        ...
        out.write("\n")
    print(json.dumps({"site_config": str(args.output.resolve()), "model_sha256": config["density"]["model_sha256"],
